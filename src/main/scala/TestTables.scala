...
object TestTables {
  ...
  def programmingLanguages1: Table = {
    ...
    val Obj = NewCol("Object-Oriented","Yes",Value(tableObjectOriented))
    val Imp = NewCol("Imperative","Yes",Value(tableImperative))
    Merge("Language",Merge("Language",Func, Obj),Imp).eval.get
  }

  // 3.2
  val programmingLanguages2: Table = {
    Filter(And(Field("Original purpose", x => x.contains("Application")),Field("Other paradigms", x => x.contains("concurrent"))),
      Value(programmingLanguages1)).eval.get
  }

  // 3.3
  val programmingLanguages3: Table = {
    val columns =List("Language", "Object-Oriented", "Functional")
    Select(columns,Value(programmingLanguages2)).eval.get
  }
